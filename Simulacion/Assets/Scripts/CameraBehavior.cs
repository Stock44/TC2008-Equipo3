...
public class CameraBehavior : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Evita que la camara valla mas alla del limite establecido en X
        if(transform.position.x < -zLimit) { transform.position = new Vector3(transform.position.x, transform.position.y, -zLimit);}
        if(transform.position.x > zLimit) { transform.position = new Vector3(transform.position.x, transform.position.y, zLimit);}
        ...
        if(Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, selectionDistance))
        {
            ...
            {follow = hit.collider.transform.GetComponent<SelectVehicle>().GetObjectToFollow();}
        }
    ...
    void LateUpdate()
    {
        if(follow != null && firstPerson == false) //Rotar la camara alrededor del obejto cuando se sigue y se esta en vista de tercera persona
        {
